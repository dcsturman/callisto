...
use crate::entity::{Vec3, DELTA_TIME };
use gomez::nalgebra as na;
use gomez::{Domain, SolverDriver, Problem, System };
...
impl FlightParams {
    pub fn new(start_pos: Vec3, end_pos: Vec3, start_vel: Vec3, end_vel: Vec3, max_acceleration: f64) -> Self {
        FlightParams { start_pos, end_pos, start_vel, end_vel, max_acceleration }
    }
}
...
impl System for FlightParams {
        ...
        Srx: na::storage::StorageMut<Self::Field, Dyn>,
    {
        ...
        let a_1: Vec3 = Vec3{ x: x[0], y: x[1], z: x[2] };
        let a_2: Vec3 = Vec3{ x: x[3], y: x[4], z: x[5] };
        let t_1 = x[6];
        let t_2 = x[7];
        // First the 3 position equations

        let pos_eqs = a_1*t_1*t_1/2.0 + a_2*t_2*t_2/2.0 + (a_1*t_1 + self.start_vel)*t_2 + self.start_vel*t_1 + self.start_pos - self.end_pos;
...
pub fn compute_flight_path(params: &FlightParams) -> (Vec<Vec3>, Vec3) {
    ...
    let v_a_1: [f64;3] = x[0..3].try_into().expect("Unable to convert to fixed array");
    let a_1: Vec3 = Vec3::from(v_a_1);

    let v_a_2: [f64;3] = x[3..6].try_into().expect("Unable to convert to fixed array");
    let a_2: Vec3 = Vec3::from(v_a_2);
    let t_1 = x[6];
    let t_2 = x[7];

    debug!("Computed path with a_1: {:?}, a_2: {:?}, t_1: {:?}, t_2: {:?}", a_1, a_2, t_1, t_2);
...
mod tests {
    ...
    fn test_compute_flight_path() {
        ...
        let params = FlightParams {
            start_pos: Vec3 { x: -2e7, y: 1e6, z: 1.5e7 },
            end_pos: Vec3 { x: 1e7, y: -2e6, z: -2.0e7 },
            start_vel: Vec3 { x: 500.0, y: 0.0, z: 0.0 },
            end_vel: Vec3 { x: 500.0, y: 0.0, z: 100.0 },
            max_acceleration: 4.0*G,
        };

        let (path, vel) = compute_flight_path(&params);

        info!("Start Pos: {:?}\nEnd Pos: {:?}", params.start_pos, params.end_pos);
        info!("Start Vel: {:?}\nEnd Vel: {:?}", params.start_vel, params.end_vel);
        info!("Path: {:?}\nVel{:?}", path, vel);

        let vel_error = (vel - params.end_vel).magnitude();
        let pos_error = (path.last().unwrap() - params.end_pos).magnitude();
